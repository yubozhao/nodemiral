 ...
 * callback: function
 */
SSH.prototype.getFile = function(src, dest, options, callback) {
  self = this;
  ...
  function onSftp(err, sftp) {
    if (err) {
      ...
      sftp.fastGet(src, dest, fastGetOptions, function(err) {
        if(err) {
          callback(err);
        } else {
          callback(null);
...
SSH.prototype.executeStream = function (shellCommand, options, callback) {
  var self = this;
  if(typeof options === 'function') {
    callback = options;
    options = {};
