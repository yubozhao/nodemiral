...
Session.prototype.download = function(src, dest, options, callback) {
  ...
  this._debug('get file - src: %s, dest: %s, vars: %j', src, dest, options.vars);

  self._withSshClient(getFile(copyFile));

  function getFile(copyFile) {
    var getFileOptions = {};
    if(options.progressBar && process.stdout.isTTY) {
      ...
      getFileOptions.onProgress = function (completedPercentage) {
        bar.update(completedPercentage / 100);
      };
    }

    return function(client, done) {
      client.getFile(copyFile, dest, getFileOptions, function(err) {
        done();
...
Session.prototype.executeStream = function(shellCommand, options, callback) {
  if(typeof(options) == 'function') {
    callback = options;
    options = {};
  }
  ...
  this._withSshClient(function(client, done) {
    client.executeStream(shellCommand, options, function(err, stream) {
      done();
      callback(err, stream);
    });
  });
